...
This is an **actively developed** project — more tools are on the way. Areas
currently in progress or planned:

- Deeper fixed-wing tuning helpers (autotune review, in-flight rate feedback).
- Broader sensor / ESC telemetry readouts.
- More guided prompts for common setup and troubleshooting flows.

Have a flight-controller task you wish Claude could do?
**[Open an issue](https://github.com/starlordz12/inav-mcp/issues)** and request a
tool — feature requests directly shape what gets built next.

## Contributing & feedback

If this saved you a bench session, here's how to help the project grow:

- ⭐ **Star the repo** so other iNAV pilots can find it.
- 🐛 **[Open an issue](https://github.com/starlordz12/inav-mcp/issues)** for bugs,
  firmware quirks, or new-tool requests.
- 🔧 **PRs welcome** — the test suite is fully offline (`pytest`, 186 tests), so
  you can add and verify a new tool without any hardware on the bench.

Developed against iNAV 8.x/9.x. Results and PRs from other targets and firmware
versions are especially welcome.
